...
# Regex to strip HTML tags when falling back to plain text
...
def send_message(
    ...
    If the first attempt with parse_mode fails due to an HTML parse error,
    ...
    # Try with requested parse_mode first; on HTML parse failure retry as plain text
    attempts = [parse_mode, None] if parse_mode else [None]
    for mode in attempts:
        payload: dict = {
            "chat_id": chat,
            "text": text if mode else _TAG_RE.sub("", text),
        ...
        try:
            ...
            if resp.status_code == 200 and data.get("ok"):
                ...
                if mode != parse_mode:
                    logger.warning(
                        f"Telegram: HTML parse failed — resent as plain text "
                        f"(id={msg_id}, {len(payload['text'])} chars)."
                    )
                else:
                    logger.info(
                        f"Telegram message sent to {chat} "
                        f"(id={msg_id}, {len(text)} chars)."
                    )
                return msg_id
            else:
                err_desc = data.get("description", resp.text[:300])
                if mode and ("can't parse" in err_desc.lower() or "parse entities" in err_desc.lower()):
                    logger.warning(
                        f"Telegram HTML parse error — retrying as plain text. Detail: {err_desc}"
                    )
                    continue  # retry without parse_mode
                logger.error(f"Telegram API error: {resp.status_code} — {err_desc}")
                return None
